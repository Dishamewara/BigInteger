...
class BigInteger {
    ...
    private:
    string number;
    bool sign; // False = +ve & True = -ve
    
    public:
        BigInteger():number("0"),sign(false){};         // empty constructor initializes zero
        BigInteger(string s);                           // "string" constructor
        BigInteger(string s,bool sn);                   // "string" constructor with sign given
...
// "string" constructor with sign given
BigInteger::BigInteger(string s,bool sn){
    number = s;
    sign = sn;
}
...
void BigInteger::setSign(bool b){
    sign = b;
}
